"""
Yield-curve-only training: **simple encoder + OU Neural SDE** (diagonal noise).

The OU variant adds mean-reverting structure to the drift:

    dz = kappa(t, z) * (theta(t, z) - z) dt + sigma(t, z) dW

Run from the repo root:

    python -m experiments_fra.one_model_experiments_on_YC.YC_SimpleEnc_OUNSDE
"""
import os
...
def main() -> None:
    ...
    # -------------------------------------------------------------------
    # Encoder — simple, bi-LSTM with layer-norm
    # -------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------
    # NSDE — OU variant: drift parameterised as kappa * (theta - z)
    # -------------------------------------------------------------------
    ...
    nsde_cfg.long_term_mean = dict(common_drift_net)
    nsde_cfg.mean_reversion = {
        **common_drift_net,
        "out_activation": "softplus",      # kappa >= 0 -> genuine mean reversion
    }
    nsde_cfg.diffusion = {
        "type": "mlp",
        "n_layers": 2,
    ...
    trainer_cfg.run_name = "YC_SOU_v1"
