"""
Yield-curve-only training: **simple encoder + simple Neural SDE** (diagonal noise).

Run from the repo root:

    python -m experiments_fra.one_model_experiments_on_YC.YC_SimpleEnc_SimpleNSDE
"""
import os
...
def main() -> None:
    ...
    # -------------------------------------------------------------------
    # NSDE — Simple Neural SDE under Q with diagonal noise
    # -------------------------------------------------------------------
    nsde_cfg = NSDECfg(type="simple", noise_type="diagonal")
    nsde_cfg.solver = "custom_euler"        # ~10x faster than torchsde on CPU
    nsde_cfg.dt = 1 / 128
    nsde_cfg.drift = {
        "type": "mlp",
        "n_layers": 3,
    ...
    nsde_cfg.diffusion = {
        ...
        "dropout": 0.1,
        "activation": "gelu",
        "out_activation": "softplus",       # keep volatility positive
    }

    model = ShortRateModel(
        name="YC_SimpleEnc_SimpleNSDE",
        encoder=encoder_cfg,
        nsde=nsde_cfg,
