...
Configuration:
- simple encoder
- simple Neural SDE under risk-neutral Q (diagonal noise)
- SimpleBondNet at delivery for the CTD futures pricer

Run from the repo root:

    python -m experiments_fra.one_model_experiments_on_YC_and_futures.YCFut_SimpleEnc_SimpleNSDE
"""
import os
...
# Bond feature width emitted by `BondMetadataStore.get_bond_features` (8 today).
BOND_FEAT_DIM = 8
...
def main() -> None:
    ...
    # -------------------------------------------------------------------
    # Data — yields + short rate + futures
    # -------------------------------------------------------------------
    data_path = "data2"
    ...
    data_cfg = DataLoaderCfg(
        ...
        enable_futures=True,            # turns on bond metadata + futures
        device=device,
    )
    dl = MarketDataLoader(data_cfg)

    # -------------------------------------------------------------------
    # Encoder — simple, bi-LSTM
    # -------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------
    # NSDE — Simple Neural SDE, diagonal noise, fast custom Euler
    # -------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------
    # BondNet — late-fusion, positive output (dirty bond prices must be > 0)
    # -------------------------------------------------------------------
    ...
    model = ShortRateModel(
        name="YCFut_SimpleEnc_SimpleNSDE",
        encoder=encoder_cfg,
        nsde=nsde_cfg,
        bondnet=bondnet_cfg,
        latent_dim=latent_dim,
    ...
    trainer_cfg.run_name = "YCFut_SS_v1"
