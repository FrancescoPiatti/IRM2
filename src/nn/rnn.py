...
class RNN(nn.Module):
    """
    Stateful vanilla RNN wrapper with a linear readout.

    Mirrors the interface of :class:`LSTM` and :class:`GRU` in this package.
    ...
    bidirectional : bool
        If True, the underlying RNN is bidirectional.
    ...
    def _build(self, input_size: int, device=None, dtype=None) -> None:
        """
        Instantiate the underlying nn.RNN and the readout. When called from
    ...
    def forward(self,
                ...
                return_sequence: bool = False) -> Union[Tensor, Tuple[Tensor, Tensor]]:
        """
        Forward pass.
        ...
        return_sequence : bool
            If True, return outputs at every timestep; otherwise return only the last step.

        Returns
        -------
        Tensor or Tuple[Tensor, Tensor]
            Network output, optionally with the final hidden state.
        """
        if x.dim() == 2:
            x = x.unsqueeze(0)
        ...
        output, new_state = self._rnn(x, state)
